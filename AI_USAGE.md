...
### 4. WebSocket Connectivity Failure with Vite/SockJS
**What the AI did wrong:** When implementing real-time updates for group settlements, the AI configured the frontend to use `SockJS` as a fallback wrapper around the STOMP protocol.
**How I caught it:** During live testing with two separate incognito windows side-by-side, the frontend failed to catch the push notifications when settlements were approved. By inspecting the browser console and the backend logs, I noticed the Vite dev server was throwing 404s trying to reach the `/ws/info` polling endpoint because Vite handles internal globals (like `window`) differently than older toolchains.
**What I changed:** I directed the AI to rip out the `sockjs-client` dependency entirely. We refactored the frontend to connect directly using Native WebSockets (`brokerURL: wsUrl`), and updated the Spring Boot `WebSocketConfig` to expose the raw endpoint without `.withSockJS()`. This instantly fixed the real-time syncing.
